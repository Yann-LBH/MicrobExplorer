...
# ==========================================================================
load_deseq2_results <- function(paths_list) {
  #' Load DESeq2 results from RDS files
  #' 
  #' @param paths_list Named list or named vector with file paths
  ...
  if (!is.list(paths_list)) {
    paths_list <- as.list(paths_list)
  }
  
  for (contrast_type in names(paths_list)) {
    file_path <- paths_list[[contrast_type]]
...
# ==========================================================================
generate_color_palette <- function(top_features, n_colors = 10) {
...
# ==========================================================================
prepare_plot_data <- function(df_abundance, sig_features, top_n = 10) {
  ...
  top_features <- df_sig[, .(Total_Abundance = sum(Abundance)), by = OTU][
    order(-Total_Abundance)][1:min(top_n, .N), OTU]
