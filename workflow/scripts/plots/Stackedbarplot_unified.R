...
TARGET_RANK <- snakemake$params$target_rank          # taxonomy mode
...
load_tsv_dir <- function(paths, select_cols = NULL) {
  rbindlist(
    lapply(paths, function(f) {
      dt <- if (is.null(select_cols)) fread(f, showProgress = FALSE)
            else fread(f, select = select_cols, showProgress = FALSE)
...
run_pathway <- function() {
  ...
  setkey(all_data, sample_id); setkey(meta, sample_id)
...
run_taxonomy <- function() {
  ...
  load_source <- function(path, source_label) {
    ...
    if (!length(files)) { warning("No files in: ", path); return(NULL) }
  ...
  top_taxa <- agg[, .(G = sum(RPKM_Sum)), by = .(Source_Type, Taxon)][
    , .SD[order(-G)[seq_len(min(TOP_N, .N))]], by = Source_Type][, .(Source_Type, Taxon)]
...
run_organisms <- function() {
  ...
  setkey(dt, sample_id); setkey(meta, sample_id)
  ...
  p_global <- ggplot(plot1_dt,
    ...
    labs(title    = sprintf("Global Abundance — Top %d Genera", TOP_N),
  ...
  p_stacked <- ggplot(plot2_dt,
    ...
    labs(title    = "Composition of Bacterial Genera by Digester",
