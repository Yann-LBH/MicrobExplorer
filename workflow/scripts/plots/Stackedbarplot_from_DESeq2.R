...
TITLE <- as.character(snakemake@params[["title"]])[1] %||% "Top N Features Differentially Abundant"
SUBTITLE <- as.character(snakemake@params[["subtitle"]])[1] %||% "DESeq2 Results"
PADJ_THRESHOLD <- as.numeric(snakemake@params[["padj"]])[1] %||% 0.05
LFC_THRESH <- as.numeric(snakemake@params[["lfc"]])[1] %||% 0
CONTRASTS_LIST <- unlist(as.character(snakemake@params[["contrasts"]])[1]) # e.g., ["ref", "date", "combo"]
